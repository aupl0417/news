...
//        $categoryPostCount = Db::name('portal_category_post')->where('category_id', $id)->count();
        $where = array('cate.category_id' => $id, 'post.delete_time' => 0);
        $categoryPostCount = Db::name('portal_category_post')->alias('cate')->join('portal_post post', 'post.id=cate.post_id', 'left')->where($where)->count();
